...
import { buildMarkdown, extractTwitterData, generateFilename } from '@/utils/core';
import './style.css';
...
(async () => {
  ...
  try {
    ...
    if (!result) {
      statusEl.textContent = 'Could not extract article'
      statusEl.className = 'error'
      return
    }
  ...
  const statusEl = document.getElementById('status')!
  statusEl.textContent = String((err as Error)?.message ?? err)
  statusEl.className = 'error'
})
